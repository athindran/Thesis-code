...
class CobraGroup(AttributePrinter):
    ...
    def __init__(self, centers):
        ...
        self.useNegativePhi = np.ones(self.nCobras, dtype=bool)
    ...
    def set_useNegativePhi(self, useNegativePhi):
        ''' set the useNegativePhi computed from TargetSelector '''
        if useNegativePhi is not None:
            self.useNegativePhi = np.array(useNegativePhi)
    ...
    def calculateElbowPositions(self, fiberPositions, indices=None):
        """Calculates the cobra elbow positions for the given fiber positions.
           Called by CollisionSimulator and CobraGroup
        ...
        """
        # Extract some useful information
        # Note: self.useNegativePhi is set at the initialization of CollisionSimulator
        centers = self.centers
        L1 = self.L1
        L2 = self.L2
        phiSign = 1 - 2*self.useNegativePhi
    ...
    def calculateCobraElbowPositions(self, cobraIndex, fiberPositions, useNegativePhi):
        """Calculates the elbow positions for a given cobra and a list of fiber positions.
           Called by TargetSelector
