...
class OnlineCollisionSimulator():
    """
    ...
    def __init__(self, bench, targets, trajectorySteps=170, trajectoryStepWidth=50, graphics = False, avertCollisions=False):
        ...
        self.avertCollisions = avertCollisions
    ...
    def run(self, solveCollisions=False):
        ...
        self.avertCollisions = False
        # Calculate the cobra trajectories
        self.simulateTrajectories()
        
        # Detect cobra collisions during the trajectory
        self.detectTrajectoryCollisions()
        [num_acq,unacquiredcobras] = self.numacq()
        #print("Colliding cobras:", self.collidingCobras)

        # Solve trajectory collisions if requested
        count = 0
        lower = True
        # Solve trajectory collisions if requested
        while solveCollisions and (self.nCollisions>0 or len(unacquiredcobras)>0) and count<4:
            print('solveCollisions')
            if count>1:
               self.avertCollisions = True 
            self.solveTrajectoryCollisions(lower)
            self.detectTrajectoryCollisions()
            [num_acq,unacquiredcobras] = self.numacq()
            lower = not lower
            count = count+1
    ...
    def optimizeUnassignedCobraPositions(self):
        ...
        for c in unassigendCobraIndices:
            ...
            distances = self.distancesToLineSegments(self.finalFiberPositions[cobraNeighbors],rotatedElbowPositions[:,None], rotatedFiberPositions[:,None])
    ...
    def defineMovementStrategies(self):
        ...
        # To do early movements for all cobras, uncomment these two lines
        # thtEarly[:] = True
        # phiEarly[:] = True
        # Save the results in a single array
    ...
    def simulateTrajectories(self):
        ...
        self.trajectories = TrajectoryGroup(nSteps=self.trajectorySteps,
                                            ...
                                            movementStrategies=self.movementStrategies,avertCollisions=self.avertCollisions,assignedCobras=self.assignedCobras,graphics=False,simulate=True)
    ...
    def numstops(self):
        return self.trajectories.numstops()
    ...
    def solveTrajectoryCollisions(self, selectLowerIndices):
        """Solves trajectory collisions changing the cobras theta movement directions.
        ...
        collidingAssociations = self.bench.cobraAssociations[:,self.associationCollisions]
        ...
        [num_acq,unacquiredcobras] = self.numacq()
        cobraIndices = np.array(np.union1d(cobraIndices,unacquiredcobras),dtype=np.int)
        #print(cobraIndices)
