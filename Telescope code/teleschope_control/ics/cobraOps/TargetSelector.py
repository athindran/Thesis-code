...
class TargetSelector(ABC):
    ...
    def calculateAccessibleTargets(self, maximumDistance=np.Inf):
        ...
        self.accessibleTargetElbows2 = np.zeros(arrayShape, dtype="complex")
        ...
        for i, indices, positions, distances in associations:
            ...
            elbows = self.bench.cobras.calculateCobraElbowPositions(
                i, positions, useNegativePhi=True)
            elbows2 = self.bench.cobras.calculateCobraElbowPositions(
                i, positions, useNegativePhi=False)
            ...
            self.accessibleTargetElbows2[i, :nTargets] = elbows2
