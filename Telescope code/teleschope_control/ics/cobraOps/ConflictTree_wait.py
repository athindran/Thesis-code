...
class Node:
    ...
    def evaluateNode(self):
       ...
       for node in allplannodes:
           #waitStartarr = np.arange(1,161,10,dtype=np.int)
           waitStartarr = np.array([1],dtype=np.int)
           nodeindex = np.argwhere(self.subgraph==node).ravel()
           waitDurationarr = np.arange(5,self.nSteps-self.subgraphCost[nodeindex],5,dtype=np.int)
           success = False
