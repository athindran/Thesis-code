       ...
       while(replan):
         ...
         for coindex,conflict in enumerate(self.conflicts):          
           ...
           if any(self.pairCollision):
              ...
              maxctime = np.max(collisionTimes)
              minctime = np.min(collisionTimes)
              sizeInterval = maxctime-minctime+1
              #sizeInterval = 5
              if self.conflicts[coindex][3]>1 and self.conflicts[coindex][4]<self.nSteps-1:
                if minctime>self.conflicts[coindex][4]:
