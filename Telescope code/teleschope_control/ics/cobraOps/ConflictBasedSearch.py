...
from .ConflictTree_bet import Node
import itertools
...
class ConflictBasedSearch():
    ...
    def run(self, solveCollisions=True):
        ...
        while solveCollisions and self.nCollisions>0 and count<4:
    ...
    def replanConflicts(self):
        # Find the time taken by all shortest paths to reach destination
        self.costs = np.array(self.alllist())
        self.detectTrajectoryCollisions()
        
        # Find connected components of all colliding cobras
        ...
        for subgraph in self.subGraphs:
          ...
          # Create a tree with a root for the subgraph
          conflictTreeRoot = Node(self.bench,subgraph,addCost,np.array([]),True,[],None,None,self.trajectories,subgraphCost)
          [unsolvedSubgraphs,addcost,stops] = self.branchTree(conflictTreeRoot,conflictTreeRoot,self.collisionPairs,self.collisionTimes,subgraph)
          self.nstops = self.nstops+stops
        #if len(unsolvedSubgraphs>0):
        #  print(unsolvedSubgraphs)
    
    #Unused. Used only for parallelizing the code.      
    ...
    def branchTree(self,currentNode,rootNode,collisionPairs,collisionTimes,subgraph):   
          ...
          # Find all pairs of colliding cobras in subgraph
          subgraphPairs = collisionPairs[:,indices]
          
          # Find startTime and endTime of collision
          ...
          # Make all the children of the current node. No. of children = 2^(No. collisions in subgraph)  
          ...
          for child in children:
            ...
            # Create childNode and evaluate it
          ...
          # Sort the child nodes based on the cost  
          currentNode.children.sort(key=lambda x:x.cost)

          for child in currentNode.children:
             # Check whether child node gives rise to additional collisions 
             ...
             if child.internalConflicts==False:
                ...
                if child.externalConflicts==False: 
                   ...
                   # If there is no external or internal conflicts, 
                   # assign the trajectories to all nodes in the subgraph and break
                   self.trajectories.tht[subgraph,:] = child.trajectories.tht[subgraph,:]
                   self.trajectories.phi[subgraph,:] = child.trajectories.phi[subgraph,:]
                   self.trajectories.elbowPositions[subgraph,:] = child.trajectories.elbowPositions[subgraph,:]
                   self.trajectories.fiberPositions[subgraph,:] = child.trajectories.fiberPositions[subgraph,:]
          ...
          # If all the children could not solve the subgraph         
          if subgraphFailure==True:
                ...
                for child in currentNode.children:
                   ...
                   if child.internalConflicts==False:
                     ...
                     # If there is an external conflict, add the additional node to the subgraph and replan.
                     cassociationCollisions,ccollisionPairs,ccollisionTimes,ccollidingCobras = self.detectCustomTrajectoryCollisions(child.trajectories)          
                     ...
                     unsolved,childCost,childStops = self.branchTree(child,rootNode,ccollisionPairs,ccollisionTimes,subgraphnew)      
                     stops = stops+childStops
                     #unsolved = []
                # If there is an internal failure, then decide that the collision can not be replanned
                if internalFailure==True or len(unsolved)>0:        
                  unsolvedSubgraphs.append(subgraphnew) 
          return unsolvedSubgraphs,addCost,stops
    ...
    def calculateTrajectories(self):
        ...
        self.trajectories = TrajectoryGroup(nSteps=self.trajectorySteps,
                                            ...
                                            movementStrategies=self.movementStrategies,avertCollisions=False,assignedCobras=self.assignedCobras,
                                            graphics=False,simulate=False)
