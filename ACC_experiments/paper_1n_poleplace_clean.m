...
rng(9);
perturb1 = rand(3,3);
Aest1 = Ad+0.2*perturb1/norm(perturb1);

perturb2 = rand(3,3);
Aest2 = Ad+1.3*perturb2/norm(perturb2);
smooth = ones(1,T);
...
for t=1:T
  ...
  if rem(t,Trob)==1
     ...
     %Knom = lqrd(Apred,Bpred,2*eye(3),0.2*eye(n),1);
     %[KROB,CL,GAM,INFO] = hinfsyn(ss(Apred,[eye(3) Bpred],[2*eye(3);zeros(n,3);eye(3)],[[zeros(3,3) zeros(3,n)];[zeros(n,3) zeros(n,n)];[zeros(3,3) zeros(3,n)]],1),3,n);
     %Krob = dlqr(Apred,Bpred,20*eye(3),0.2*eye(n));
     %Krob = dlqr(Apred,Bpred,20*eye(3),0.2*eye(n));
     %Krob = -1*INFO.Ku;
     clvalues = [0.5+0.3i;0.5-0.3i;-0.5];
     Krob = place(Apred,Bpred,clvalues);
