...
sysdest = c2d(ss(Acest,Bcest,Cc,Dc),1);
Aest = sysdest.A;
Best = sysdest.B;

%Aest = Ad+0.1*rand(4,4);
%Best = Bd+0.1*rand(4,1);
...
for t=1:T
  ...
  % Designing the controller
   if rem(t,Topt)==1
     ...
     %Knom = dlqr(Apred,Bpred,20*eye(d),0.2*eye(n));
     ...
     Knom = -1*INFO.Ku;
     %Knom = lqrd(Apred,Bpred,2*eye(d),0.2*eye(n),1);
  ...
  %xt(1) = xt(1)+0.1*xt(3);
  %xt(2) = xt(2)+0.1*xt(4);
  %xt(3) = xt(3)+0.1*(-xt(1)-deltat*xt(3)+u);
  %xt(4) = xt(4)+0.1*(-sin(xt(2))-deltat*xt(4)+rt*u)/Jt;
  yt = Cd*xt+et(t);
  yerror = xt-xpred;
  yerrs(t) = norm(yerror);
  states(t,:) = yt;
...
xlabel('Time (t)');
ylabel('\theta');
legend('Cons','Agg','CVM control');
title('Sum of Sinusoids tracking');

subplot(1,2,2)
plot(log10(abs(states(:,1)'-ref(1,1:T))),'r-');
hold on;
plot(log10(abs(states2(:,1)'-ref(1,1:T))),'b-');
hold on;
plot(log10(abs(states3(:,1)'-ref(1,1:T))),'color',[0.4 0.6 0.2]);
ylim([-5,1])
grid on;
xlabel('Time (t)');
...
saveas(gcf,'./general_figures/spring_345n.eps','epsc');
